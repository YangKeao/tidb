...
func (s *session) SetGlobalSysVar(ctx context.Context, name, value string) (err error) {
	sv := variable.GetSysVar(name)
	if sv == nil {
		return variable.ErrUnknownSystemVar.GenWithStackByArgs(name)
	}
	...
	if err = sv.SetGlobalFromHook(ctx, s, value, false); err != nil {
		return err
	}
...
func (s *session) SetGlobalSysVarOnly(ctx context.Context, name, value string) (err error) {
	sv := variable.GetSysVar(name)
	if sv == nil {
		return variable.ErrUnknownSystemVar.GenWithStackByArgs(name)
	}
	if err = sv.SetGlobalFromHook(s.currentCtx, s, value, true); err != nil {
		return err
	}
	if sv.HasInstanceScope() { // skip for INSTANCE scope
		return nil
