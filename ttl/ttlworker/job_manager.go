...
func updateHeartBeatSQL(tableId int64, id string) string {
	now := time.Now()
	return fmt.Sprintf(updateHeartBeatTemplate, now.Format(timeFormat), id)
}
...
func NewJobManager(id string, sessPool session.SessionPool) (manager *JobManager) {
	manager = &JobManager{}
	manager.id = id
...
func (m *JobManager) UpdateHeartBeat(ctx context.Context, se session.Session) error {
	for _, job := range m.localJobs() {
		_, err := se.ExecuteSQL(ctx, updateHeartBeatSQL(job.tbl.ID, m.id))
		if err != nil {
