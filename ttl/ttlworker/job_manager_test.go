...
func (p *singleSessionPool) Get() (pools.Resource, error) {
	p.Lock()
	return p.session, nil
}

func (p *singleSessionPool) Put(pools.Resource) {
	p.Unlock()
}
...
func TestSyncInfoSchemaAndTTLStatus(t *testing.T) {
	...
	m := ttlworker.NewJobManager("test-id", &singleSessionPool{session: ttlSession})
...
func TestUpdateHeartBeat(t *testing.T) {
	...
	tk.MustExec("insert into mysql.tidb_ttl_table_status(table_id, parent_table_id, current_job_owner_id) values (1, 2, 'test-id')")
	assert.NoError(t, m.UpdateHeartBeat(context.Background(), ttlSession))
	rows := tk.MustQuery("select table_id, parent_table_id, current_job_owner_hb_time from mysql.tidb_ttl_table_status").Rows()
	assert.Equal(t, 1, len(rows))
	assert.Equal(t, fmt.Sprint(1), rows[0][0])
	assert.Equal(t, fmt.Sprint(2), rows[0][1])
	hbTime, err := time.Parse("2006-01-02 15:04:05", rows[0][2].(string))
