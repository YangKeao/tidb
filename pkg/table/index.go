...
type Index interface {
	...
	// MeetPartialCondition returns true if the row meets the partial condition of the index.
	MeetPartialCondition(row []types.Datum) (bool, error)
	// MeetPartialConditionWithChunk returns true if the row meets the partial condition of the index.
	// It has the same function with `MeetPartialCondition`, but it accepts a chunk.Row instead of a slice of datums.
	MeetPartialConditionWithChunk(row chunk.Row) (bool, error)
	// Create supports insert into statement.
	// The `Create` inserts the index without considering the partial condition. The caller should call `MeetPartialCondition` to check whether the
	// row meets the partial condition before calling `Create` to avoid unnecessary index creation.
	...
	// The `Delete` deletes the index without considering the partial condition. The caller should call `MeetPartialCondition` to check whether the
	// row meets the partial condition before calling `Delete` to avoid unnecessary index deletion.
	...
	// `GenIndexKVIter` doesn't consider the partial condition, the caller should call `MeetPartialCondition` to check. If the row doesn't meet
