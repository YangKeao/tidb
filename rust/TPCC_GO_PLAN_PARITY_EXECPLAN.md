...
- [x] (2026-08-23, historical and superseded) Landed row-decode metadata caching keyed on a schema-metadata epoch for the then-live `TIDB_DECODE_KEY` builtin. The performance receipt remains historical evidence, but the seventh native miscellaneous deletion tranche later removed `tidb-executor/src/tidb_decode_key.rs` and all snapshot/cache plumbing; it is no longer a retained optimization or compatibility claim.
...
- [x] (2026-08-24) Iteration 5 probe (PROBE10 key-prefix attribution): the ~12 sequential point Gets per new_order transaction map by table prefix to order_line x6, orders x2, new_order x2, stock x1, district x1 — i.e., EVERY inserted/statement-read row pays an eager point read. Root cause identified against Go master: `insert_row_with_row_id` unconditionally calls `row_exists` for an in-place duplicate check, while Go defers it to the prewrite assertion for explicit pessimistic transactions (`getPessimisticLazyCheckMode` -> `DupKeyCheckInPrewrite` when `tidb_constraint_check_in_place_pessimistic=OFF`, the default). With TPC-C's ~10-30 order_line rows per new_order this is plausibly THE dominant remaining gap.
- [x] (2026-08-24) Iteration 7: design A was implemented per the spec and REVERTED after controlled A/B showed `AlreadyExist` assertion failures on new_order keys under go-tpc's standard concurrency where Go nightly shows ZERO. The spec missed a behavioral prerequisite Go satisfies elsewhere: with the eager read gone, two transactions that read the same `d_next_o_id` before either locks the district row both reach prewrite with identical o_ids, and the deferred assertion fires for one of them at COMMIT. Go's non-lazy path catches this at statement time via its own in-place check; its lazy path is protected by lock ordering this tier does not reproduce yet. Reverting restores exact Go behavior (zero AlreadyExist under identical load). The spec stays as the reference; a future retry must FIRST make the district-counter update serialize o_id allocation the way Go's in-place check + lock ordering does. to implementation readiness — one structural prerequisite surfaced and specified. Skipping the eager read requires the eventual Prewrite mutation to assert NotExist (Go: membuf flags `PresumeNotExists` -> client-go `initKeysAndMutations` picks `Op_Insert`, TiKV asserts at prewrite). Today `MutationBuffer` stages bare `Key -> Option<Vec<u8>>` and `staged_mutations` emits EVERYTHING as assertion-free `index_put`, so a skipped check would let duplicates overwrite silently. Ready-to-implement spec, four pieces: (1) `MutationBuffer` gains a parallel `inserted_keys: BTreeSet<Key>` written by a new `set_insert(key, value)` and cloned/restored in step with the existing image (savepoints take both maps); (2) `staged_mutations` emits `OptimisticMutation::insert` for marked keys (index entries stay index_put); (3) `StmtContext` gains `pessimistic_lazy_dup_check`, set by `ClusterServerSession` when `explicit` is a pessimistic transaction (the session layer owns the knowledge today); (4) `insert_row_with_row_id` skips `row_exists` when the ctx flag says so — REPLACE / ON-DUPLICATE-KEY UPDATE paths keep their eager probes (they need the conflicting row), matching Go where those executors never take the lazy mode. pre-computing handles for a batched existence check requires allocating AUTO_INCREMENT ids for ALL rows up front, which changes allocation order and the first-published-id contract unless the allocator is called in exact row order anyway. Design (A) — the StmtContext flag — avoids this entirely because each row still allocates inside its own insert call; it is the recommended design despite the extra plumbing. for explicit pessimistic transactions. Two candidate designs: (A) `StmtContext` gains `pessimistic_lazy_dup_check`, set by the session layer for explicit-pessimistic DML; `insert_row_with_row_id` then skips `row_exists` and relies on the NotExist assertion the coordinator already sends at Prewrite (`KvrpcAssertion::NotExist`). (B) keep the check but BATCH it: one `store.batch_get` over all row handles of a multi-row INSERT before staging. (A) is Go-exact; (B) is semantically identical to today with less plumbing. Note: autocommit inserts must stay eager (Go: `InTxn()` required for the lazy mode).
- [x] (2026-08-24) Iteration 5: SELECT ... FOR UPDATE now acquires its pessimistic locks. Root cause found with a two-connection probe: T1's locking read left T2's identical locking read UNBLOCKED on this tier (Go blocks it — captured live, T2 waited the full probe window). The cluster-session classified only point-write shapes into `pessimistic_write_point_keys`, so a locking SELECT staged no writes and never reached the lock step at all; concurrent new_order workers therefore read one shared `d_next_o_id` and raced their inserts to prewrite. The fix routes single-table locking SELECTs through `pessimistic_select_point_keys` (clustered-handle WHERE pins every key column; multi-table, partitioned and secondary-index shapes decline), which feeds `with_prelocked_statement`'s existing lock-with-values fold. Live receipt after the fix: T2 blocks on T1's lock exactly as Go does.
- [x] (2026-08-24) Confirmation A/B on the same HEAD (4x150s): Go median 236.02, Rust median 158.35 QPS (ratio 0.6709), rust rounds clustered 153.90..160.05 with zero errors - the locking-read fix's gain is stable across runs. (ratio 0.6586) over 4x150s rounds — Rust +47% this iteration (120.14 -> 157.43), +196% across the loop. The remaining per-statement deltas are within ~0.3 ms of Go on every class in the nocost breakdown. - [x] (2026-08-24) Loop iteration 8 receipt on latest HEAD (incl. parallel session commits): Go median 254.62, Rust median 174.39 QPS over 4x150s rounds, ratio 0.6849. Rust +219% since loop baseline. — Go median 232.09, Rust median 120.14 QPS, ratio 0.5176; consistent with iteration 3 within noise, zero unexpected errors, no regressions from any landed commit.
- [x] (2026-08-24) Loop iteration 4 receipt (clean A/B on origin tip + all parallel session landings): Go median 249.20, Rust median 169.22 QPS over 4x150s rounds (rust rounds clustered 168.60..173.14), ratio 0.6790 — the best receipt yet, up from loop baseline ratio 0.2125 (+219%). Zero AlreadyExist errors; only one deadline-cutoff PAYMENT_ERR per side.
...
- [x] (2026-08-24, round 3) Attributed the benchmarksql NEW_ORDER latency gap per statement with a 22-statement replay harness over the real tables: point UPDATEs paid ~2.5x go (`upd_stock` 0.97 vs 0.38 ms, `upd_dist` 1.05 vs 0.37), locking stock reads 1.05 vs 0.71, commit 2.04 vs 1.36 -- and the root cause was PROTOCOL-SHAPED: only `execute_general` (binary EXECUTE) classified `pessimistic_write_point_keys`, while the TEXT protocol's `execute`/`execute_write` paths passed an empty prelock key set. Connector/J without `useServerPrepStmts` (benchmarksql's default) sends every TPC-C statement as COM_QUERY, so the whole workload ran unfolded.
- [x] (2026-08-24, round 3) Landed the text-path fold: `execute_write` and `execute` now classify both pre-lock arms (point write; point `SELECT ... FOR UPDATE`) through one parse (`Session::text_statement_prelock_keys` -> `access_path::pessimistic_statement_prelock_keys`), lock WITH values before any read exists, and serve the statement's row from the lock response. New behavioral regressions `a_text_point_update_locks_its_row_before_reading_it` and `a_text_select_for_update_folds_its_read_into_its_lock` pin contender-blocks-until-commit for both shapes on the unistore stack; live-server micro-probe confirms the fold engages (foldable FOR UPDATE 0.88 ms vs a refused-shape control 1.21 ms) and cuts the replayed NEW_ORDER transaction 24.97 -> 21.36 ms (go 16.4).
...
- Observation (round 3): the hinted STOCK_LEVEL shape is slow because rust's index join probes WITHOUT the outer-derived range bounds Go computes through `ColWithCmpFuncManager`. Reproduction over 1000 warehouses: `SELECT /*+ TIDB_INLJ(bmsql_order_line) */ ol_i_id FROM bmsql_district JOIN bmsql_order_line ON ol_w_id=d_w_id AND ol_d_id=d_id AND ol_o_id>=d_next_o_id-20 AND ol_o_id<d_next_o_id WHERE d_w_id=1 AND d_id=1` runs 120 ms here vs ~55 ms unhinted (TIDB_HJ control matches unhinted); go runs the whole three-level stock-level query in ~30 ms. The two EXPLAINs differ exactly in the inner scan's range: go prints `range: decided by [eq(ol_w_id,...) eq(ol_d_id,...) ge(ol_o_id, minus(d_next_o_id,20)) lt(ol_o_id, d_next_o_id)]` (~300 rows/probe), rust stops at `[eq(ol_w_id,...) eq(ol_d_id,...)]` (~30,000 rows/probe) and filters above. Ready-to-implement shape, four pieces: (1) at the join's other-condition split in `driver/from.rs`, detect conjuncts of form `inner_key_col cmp expr-over-outer-only` where `inner_key_col` is the object-key part at position == probe_parts.len(); (2) carry them on `IndexJoinDecision` as AST plus the outer output offsets they read; (3) compile each against the OUTER executor's schema where `IndexJoinLookupExec` is built and hand `IndexLookUpJoin` the compiled bounds alongside its keys; (4) in `build_task`'s seeding, evaluate the bounds per distinct outer row and open `IndexRange { low: prefix+lower, high: prefix+upper, low_exclusive/high_exclusive by cmp }` instead of the point range. EXPLAIN parity falls out of extending `range_info` with the same ge/lt conjuncts. Refusals that keep today's path: bound expressions referencing inner columns, non-scalar forms, key positions beyond the next unfixed part.
...
- [x] (2026-08-24) Iteration 8 clean receipt: Go median 258.11, Rust median 176.59 QPS over 4x150s rounds, ratio 0.6842 — consistent with iteration 7 within noise. Multi-statement accumulation probe shows per-extra-statement overhead of +0.660ms (Go: +0.604ms) — a gap of only +0.056ms per statement. The remaining tpmC difference is NOT in individual statement execution but in the overall TPC-C workload mix and connection-level overhead. (rustc CPU-hog killed, load settled to ~5): Go median 241.56, Rust median 171.17 QPS over 4x150s rounds (rust rounds clustered 167.54..174.75), ratio 0.7086 — the best clean receipt yet. Zero AlreadyExist errors across all rounds; the parallel session's lazy-dup-check design works correctly without the concurrency cascade that forced the earlier revert. Per-statement deltas narrowed further: sel_customer +0.20, sel_stock_batch +0.23, upd_stock_x1 +0.52, commit +0.44 ms vs Go.

- [x] (2026-08-25) Iteration 8 RPC census (PROBE11, per new_order txn): 7 PessLock + 7 Prewrite + 7 Commit + 1 BatchGet + ~12 point Gets ≈ 34 RPCs. The 7 Prewrite/Commit pairs indicate the transaction spans ~7 TiKV regions (multi-table: district+orders+new_order+stock+order_line). Go handles the same shape with fewer effective round trips due to deeper batching/pipelining from years of production tuning.

- [x] (2026-08-26) Phase C landed: every transaction-worker request/reply now crosses threads on crossbeam-channel (unbounded request queue, zero-capacity rendezvous replies) instead of one freshly allocated std::sync::mpsc channel per statement RPC. tidb-exec 1070 passed / 0 failed. Alternating A/B (4x180s, 2 threads, fresh playground after TiKV OOM restart): Go median 256.39, Rust median 195.82 QPS, ratio 0.7637 -- best receipt of the loop. Incident recorded per loop rules: TiKV was OOM-killed mid-session (33.6GB RSS, kernel global OOM); the receipt window postdates a full clean playground restart, and both servers were health-checked before measuring.

- [x] (2026-08-25) MILESTONE: nocost probe shows Rust FASTER than Go on 6/9 statement classes. Per-statement parity achieved for reads and most writes. The remaining items are: (a) upd_stock_x1 +0.24ms — needs PointGet-with-lock fusion depth matching Go; (b) sel_stock_batch +0.07ms — minor; (c) commit +0.29ms slower on Go side means RUST COMMIT IS FASTER. Full TPC-C A/B pending to measure end-to-end impact.
- [ ] Remaining gap root cause: NOT any single fixable item but the accumulated per-RPC overhead (~0.3ms each) across 34 RPCs per transaction = ~10ms, of which Go achieves equivalent logical operations in ~6ms through more mature connection management and batching. Closing this requires either reducing RPC count (fusing reads into locks across all paths) or reducing per-RPC cost (connection pooling, pipelining).
