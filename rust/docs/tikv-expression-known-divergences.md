...
| 8 | `CONCAT`, `CONCAT_WS`, `REPEAT`, `SPACE`, `LPAD`, `RPAD`, `INSERT`, `TO_BASE64`, `FROM_BASE64`, `MAKE_SET` | context | TiKV has no `max_allowed_packet` seam; TiDB warns/errors and truncates | `context.rs` `max_allowed_packet` contract | All stay native |
