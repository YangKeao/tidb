...
- 2026-09-07 (`br/pkg/restore/utils` return contracts): the complete six-module
  ...
  callers, and non-source build surfaces were inventoried without reopening Go
  source. Fifteen direct Go-shaped results no longer impose Rust-only
  diagnostics; the focused regression failed with exactly fifteen diagnostics
  before the edit and passes afterward. The shared crate passes 34 active
  tests with two unrelated skips, all-target compilation and scoped formatting
  pass, and evidence is in `receipts/br_restore_utils.md` and its ExecPlan.
...
- 2026-09-06 (`pkg/owner` API return contracts): the complete eight-artifact,
  ...
  master. Seven direct Go-shaped APIs no longer emit Rust-only `#[must_use]`
  diagnostics; seven Rust adapter/helper annotations remain. A focused
  regression failed with exactly seven diagnostics before the edit and passes
  afterward; the twelve-test owner suite and all-target check pass. Evidence
  is recorded in `receipts/owner.md`.
...
- 2026-09-06 (`pkg/util/topsql/stmtstats` Go-shaped constructors and
  ...
  Rust-only map/field conveniences remained annotated. Focused deny-on-discard
  regressions failed with exactly fourteen diagnostics on the detached
  pre-fix owner and pass with all 42 stmtstats tests, the util all-target
  check, formatting, lint, and diff checks. Evidence is recorded in
  `receipts/util_topsql_audit.md`.
...
- 2026-09-05 (`pkg/sessionctx/variable` scatter-region validation): matched
  ...
  SQL regression covers SESSION/GLOBAL readback, normalization, and
  preservation after refusal; the remaining varsutil validation matrix stays
  explicitly bounded.
...
- 2026-09-05 (`pkg/sessionctx/variable` default validation): added a
  registry-wide regression that runs every writable default through the Go
  type/closure validation contract for each advertised scope and requires the
  canonical value to remain unchanged; removed the stale vardef gap shim.
- 2026-09-05 (`pkg/sessionctx/variable` plan-replayer retention bound): fixed
  the Rust catalog's missing one-year `TypeDuration` maximum so the canonical
  seven-day default remains settable and over-bound writes clamp to Go's
  `8760h0m0s`; the focused duration regression now pins that boundary.
- 2026-09-05 (`pkg/sessionctx/variable` setter scope contract): added a
  registry-wide regression over the live Rust SESSION/GLOBAL routes, proving
...
- 2026-09-05 (`pkg/ddl` ALTER ADD FOREIGN KEY validation): Rust now rejects
  ...
  `receipts/ddl_foreign_key_alter_validation.md`.
- 2026-09-05 (`pkg/ddl` ALTER foreign-key atomicity): Rust now stages every
  ...
  `receipts/ddl_foreign_key_alter_atomicity.md`.
- 2026-09-05 (`pkg/ddl` foreign-key partial-index safety): Rust now preserves
  non-partitioned partial-index predicates as compiled table metadata and
  evaluates them during backfill, insert, update, delete, and drop-index
  maintenance. `IS NOT NULL` predicates on every foreign-key column are
  treated as safe coverage, while unsafe child predicates cause Go-compatible
  auto-support-index creation and 1553 protection of the actual support key;
  parent-side partial indexes retain the Go 1452 orphan check. The focused
  three-shape regression covers safe child, unsafe child, and safe parent
  predicates with exact 1451/1452/1553 outcomes. Evidence is recorded in
...
- 2026-09-05 (`pkg/util/parser`): the complete `tidb-parser` util owner now
  matches Go's discardable-return contract by removing nine explicit Rust-only
...
- 2026-09-05 (`pkg/util/rowDecoder`): the dependency-closed row decoder owner
  now matches Go's discardable-return contract by removing three explicit
...
- 2026-09-05 (`pkg/util/table-rule-selector`): the complete selector owner
  now matches Go's discardable-return contract by removing the explicit
  Rust-only `#[must_use]` diagnostic from `TrieSelector::new`. The
...
- 2026-09-05 (`pkg/util/versioninfo`): the complete versioninfo owner now
  matches Go's discardable-return contract by removing the explicit Rust-only
  `#[must_use]` diagnostic from `tidb_edition`. The deny-on-discard regression
...
- 2026-09-08 (`pkg/planner/util/tablesampler` return contracts): current Go
  master `aec988ea500d` was re-read as the complete two-artifact, 74-line
  package, including its table-sample metadata and memory-accounting source,
  BUILD target, and absence of tests, fixtures, generated/platform variants,
  and nested packages. The dependency-closed `table_sampler.rs` owner, inline
  owner test, and direct planner/read-only-scan consumers were inventoried.
  Two Rust-only `#[must_use]` diagnostics were removed from the Go-shaped
  metadata constructor and memory query. The focused deny-on-discard
...
- 2026-09-06 (GROUP BY expression panic recorded, NOT FIXED): `select v % 3
  ...
  the null bitmap. ROOT CAUSE (runtime probe at parallel.rs:1453): the
  cop-tier parallel pipeline's output carries ONLY finished agg results —
  each group's GROUP-BY columns are never emitted (the serial path survives
  only because planners usually make a `firstrow(group_col)` aggregate
  double as the group column; expression group keys break that). Fix: the
  parallel final output must prepend the DECODED group-by datums per group
  (decode each group's encoded key per the group_by expressions' field
  types, mirroring Go's final worker which writes group columns into the
  chunk before the agg results), and the reader schema/indexes must align
  with that layout.
