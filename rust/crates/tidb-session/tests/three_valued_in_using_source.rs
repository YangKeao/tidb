//! NOT IN with NULL remains executable; former IN and USING-join values are
//! retained as independent oracles for their explicit local-ISNULL contractions.
...
fn not_in_null_semantics_by_case() {
    ...
    for (name, right, predicate, expected) in [
        ...
        (
            "exists",
            "(2),(NULL)",
            "not exists (select 1 from s where k=a)",
            "1;3;4",
        ),
    ] {
        ...
        session
            .run("insert into t values (1,1),(2,2),(3,3),(4,NULL)")
            .unwrap();
        session.run("create table s (k int)").unwrap();
        if !right.is_empty() {
            session
                .run(&format!("insert into s values {right}"))
                .unwrap();
        }
        ...
        if actual != expected {
            failures.push(format!(
                "{name}: expected {expected:?}, got {actual:?}; plan: {plan}"
            ));
        }
    }
...
fn tuple_not_in_false_dominates_unknown() {
    let mut session = Session::new();
    session
        .run("create table t (id int, a int, b int)")
        .unwrap();
    session
        .run("insert into t values (1,1,1),(2,2,1),(3,NULL,2),(4,NULL,1)")
        .unwrap();
...
#[test]
fn not_in_survives_while_in_and_using_contract() {
    let mut session = Session::new();
    ...
    assert_eq!(
        rows(
            &mut session,
            "select a from t where a not in (select k from s)"
        ),
        ""
    );
    // IN formerly kept the concrete match.
    ...
    session
        ...
        .unwrap();
    session
        .run("insert into r values (2, 99), (3, 30)")
        .unwrap();
    crate::assert_removed_misc(
        &mut session,
        "select id, v, w from l join r using (id) order by id",
        "2|20|99",
    );
}
