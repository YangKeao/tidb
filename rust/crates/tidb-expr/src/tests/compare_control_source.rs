...
/// TestCompareFunctionWithRefine` over its 35-row table.
///
...
fn test_compare_function_with_refine() {
    let rows: [(&str, &str); 35] = [
        ...
        (
            "a != '1.1'",
            "ne(cast_double(col(Some(Long))), Const:FLOAT:1.1)",
        ),
        ...
        (
            "'1.1' != a",
            "ne(Const:FLOAT:1.1, cast_double(col(Some(Long))))",
        ),
...
fn test_compare() {
    ...
    // - `<datetime column> <cmp> <const>`: the comparison builder folds the
    //   constant cast through its statement context.
...
fn test_refine_args_with_nullable_column() {
    ...
    assert_eq!(
        shape(&built),
        "eq(Const:UINT:9223372036854775808, col(Some(Long)))"
    );
}
...
fn test_refine_args_with_cast_enum() {
    ...
    assert_eq!(shape(&built), "eq(Const:UINT:0, col(Some(Enum)))");
}
