...
fn builtin_return_type_before_ret_tp(name: &str, args: &[Expression]) -> Option<FieldType> {
    ...
    Some(match name {
        ...
        "sign" => int(),
        // Go `crc32FunctionClass.getFunction` adds `mysql.UnsignedFlag`, and
        // `newBaseBuiltinFuncWithTp` stamps every integer result with
        // `mysql.BinaryFlag`. The kernel already returns an unsigned datum, so
        // a signed declaration here is what made the engine's value look
        // different.
        "crc32" => {
            let mut ft = int();
            ft.add_flags(
