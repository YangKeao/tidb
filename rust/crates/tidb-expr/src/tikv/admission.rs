...
pub(crate) const ADMISSION_ROWS: &[AdmissionRow] = &[
    ...
    row("last_day", Decision::Excluded, Signature::None, &[], Shape::Any, "LAST_DAY over an implicit temporal cast returns a shape the exact bridge refuses, so native answers where the engine errors (dual-run harness)"),
