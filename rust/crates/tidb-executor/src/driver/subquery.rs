...
/// Every column `expr` reads outside an aggregate, window call, subquery, or
/// `ANY_VALUE`/`GROUPING` subtree.
pub(crate) fn bare_columns(expr: &tidb_ast::Expr) -> Vec<Vec<String>> {
    ...
    impl Visitor for Collector {
        fn enter(&mut self, node: &mut dyn Any) -> bool {
            ...
            match expr {
                ...
                | tidb_ast::Expr::GroupConcat { .. }
                | tidb_ast::Expr::Window { .. }
                | tidb_ast::Expr::Subquery(_)
                | tidb_ast::Expr::Exists { .. } => true,
    ...
    Visitable::accept(&mut owned, &mut collector);
    collector.found
}
...
fn collect_correlated_columns(
    ...
    let inner = select.from.as_ref().map_or_else(
        || FromScope::for_statement(ctx),
        |join| {
            super::planner_bridge::logical_from_scope(join, catalog, current_db, ctx)
                .unwrap_or_else(|_| FromScope::for_statement(ctx))
        },
    );
...
pub(crate) fn select_outer_scope(
    ...
    select.from.as_ref().map_or_else(empty, |join| {
        super::planner_bridge::logical_from_scope(join, catalog, current_db, ctx)
