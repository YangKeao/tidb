...
fn build_window(
    ...
    for (index, descriptor) in window.window_func_descs.iter().enumerate() {
        ...
        let func = match descriptor.base.name.to_ascii_lowercase().as_str() {
            "row_number" => WindowFunction::RowNumber,
            "rank" => WindowFunction::Rank { dense: false },
            "dense_rank" => WindowFunction::Rank { dense: true },
            "percent_rank" => WindowFunction::PercentRank,
            "cume_dist" => WindowFunction::CumeDist,
            "first_value" | "last_value" | "nth_value" => {
                let name = descriptor.base.name.as_str();
                let arg = descriptor
                    .base
                    .args
                    .first()
                ...
                WindowFunction::Value {
                    arg: resolve_expression(arg, &child_schema)?,
                    nth,
                    last: name == "last_value",
                }
            }
            "lead" | "lag" => {
                let args = &descriptor.base.args;
                ...
                WindowFunction::Relative {
                    ...
                    offset,
                    default: default
                        .map(|expr| resolve_expression(expr, &child_schema))
                        .transpose()?,
                    lead: descriptor.base.name == "lead",
                }
            }
            "ntile" => {
            ...
            _ => {
                let descriptor = AggFuncDesc::new_for_window_func(descriptor, false);
                WindowFunction::Aggregate(aggregate_function(&descriptor, &child_schema, ctx)?)
            }
        };
...
fn window_frame_spec(
    ...
    if frame.frame_type == FrameType::Groups {
        return Err(DriverError::unsupported(
            "GROUPS window frames are not supported by TiDB",
        ));
    }
    ...
    Ok(crate::window::WindowFrameSpec {
        ...
        range: (frame.frame_type == FrameType::Ranges).then(|| frame.clone()),
        range_desc,
    })
}
...
fn join_output_offsets(
    ...
    output_schema
        ...
        .iter()
        .map(|output| {
            usize::try_from(output.index)
                .ok()
                .filter(|index| {
                    merged_columns
                        .get(*index)
                        .is_some_and(|column| column.unique_id == output.unique_id)
                })
                .ok_or_else(|| {
                    DriverError::unsupported(
...
fn build_index_inner_subtree(
    ...
    match plan {
        ...
        PhysicalPlan::HashJoin(hash_join) => {
            ...
            executor
                .set_default_values(hash_join.default_values.clone())
...
fn build_index_join(
    ...
    executor
        ...
        .map_err(DriverError::Exec)?;
    executor
        .set_default_values(join.default_values.clone())
        .map_err(DriverError::Exec)?;
    Ok(Box::new(executor))
}
...
fn build_join_over_children(
    ...
    // ON predicates on a preserved side control matching, not whether that
    // row exists. Go installs these as OuterFilter and emits failed rows
    // through the joiner's unmatched path.
    ...
    let preserve_right = join_type == LogicalJoinType::RightOuter;
    let left = filtered_join_child(
        plan,
        left,
        if preserve_left { &[] } else { left_conditions },
        ctx,
    )?;
...
mod tests {
    ...
    fn table_reader_build_records_go_stmtctx_table_access() {
        ...
        cancellation.cancel();
        let ctx = crate::StmtContext::for_query_with_memory(session.statement());
