...
//! session thread. Inner joins evaluate arbitrary residual conditions on those
//! workers through the normal expression evaluator, as Go does. Parallel
//! build, duplicate/non-integer keys, residuals for the preserved join
//! families, v2's partitioned spill, and outer-apply remain deferred.
//! Hash-aggregate spill, TopN spill, parallel-sort spill and
...
struct HashState {
    ...
    /// Results produced by the exact-integer probe workers, released in
    ...
    /// `probeChkResourceCh` / `joinChkResourceCh`. These two pools are the
    /// same ownership loop without exposing the non-`Send` executor tree to
    /// worker threads.
    ...
    /// Number of probe chunks handed to pool workers by the parallel
...
/// expression context and `OtherConditions` to every probe worker; these are
/// the same shared ownership boundary, taken once per probe phase.
struct ParallelProbeShared<C> {
    output_layout: JoinOutput,
    table: Arc<BuildTable>,
    build_types: Vec<FieldType>,
...
/// Go `hash_join_v2.go`'s probe pipeline, with the session thread in the roles
/// of `fetchProbeSideChunks` (it alone drives the probe child) and `Next` (it
/// alone consumes `joinResultCh`), and one pool task per probe chunk in the
/// role of `runJoinWorker`. The chunks in circulation are bounded exactly as
/// Go bounds them: `Concurrency` probe chunks (`probeChkResourceCh`) and one
/// result chunk per worker plus the one the parent holds
/// (`joinChkResourceCh`, `joinResultCh`). A task ends with its chunk, so an
/// idle lane never occupies a pool worker while a nested join or aggregate
...
struct IndexLookupState {
    unordered: Option<IndexUnordered>,
    hash: Option<IndexHashState>,
    hash_results: Option<std::sync::mpsc::Receiver<IndexTaskOutcome>>,
    hash_chunk: Option<IndexHashChunk>,
    hash_task_bytes: i64,
    /// The current batch's outer rows in child order. Ordinary lookup and
    /// ordered hash joins preserve it; unordered hash joins probe inner rows.
    outer: OuterBatch,
    /// How far `outer` is consumed.
    cursor: usize,
    /// Go's innerIter/hasMatch, retained when one outer row spans requests.
    probe: Option<IndexRowProbe>,
    condition_scratch: Option<Chunk>,
...
enum PendingIndexLookupSource {
    /// Go `innerWorker.handleTask`: a lane thread builds lookup contents,
    /// fetches inner chunks and builds their map while earlier tasks drain.
    Draining {
...
/// Hash variants also own ON evaluation/output; ordinary lookup does not.
struct IndexTaskShared<C: Columns> {
    hash_output: Option<IndexHashOutput>,
...
/// Go innerWorker.handleTask: prepare lookup content, inner rows and map.
fn run_index_task<C: Columns>(shared: &IndexTaskShared<C>, outer: OuterBatch) -> IndexTaskOutcome {
    ...
    fetched_index_task_outcome(outer, fetched)
}
...
fn fetched_index_task_outcome(
    outer: OuterBatch,
    fetched: Result<FetchedIndexTask, ExecError>,
) -> IndexTaskOutcome {
    match fetched {
        Ok(FetchedIndexTask::Prepared { inner, inner_rows }) => IndexTaskOutcome::Prepared {
            outer,
            inner,
            inner_rows,
        },
        Ok(FetchedIndexTask::Unforked(probes)) => IndexTaskOutcome::Unforked { outer, probes },
        Err(error) => IndexTaskOutcome::Failed {
            outer,
            error: Box::new(error),
        },
    }
...
impl PreparedIndexInner {
    ...
    fn install(mut self, state: &mut IndexLookupState) {
        ...
        state.matched = std::mem::take(&mut self.matched);
        state.inner_bytes = std::mem::take(&mut self.bytes);
    }
}

impl Drop for PreparedIndexInner {
    fn drop(&mut self) {
        self.tracker.consume(-self.bytes);
    }
}

/// Go fetchInnerResults/buildLookUpMap, without constructing joined rows.
fn prepare_index_inner<C: Columns>(
    shared: &IndexTaskShared<C>,
    prefetched: Vec<Chunk>,
) -> Result<PreparedIndexInner, ExecError> {
    let mut prepared = PreparedIndexInner {
        rows: List::new(&shared.inner_types, shared.init_cap, shared.max_chunk_size),
...
fn build_index_lookup_map(
    inner: &List,
    keys: &[EquiKey],
    inner_types: &[FieldType],
    outer_is_left: bool,
) -> Result<FastBytesMap<Vec<RowPtr>>, ExecError> {
    ...
    for chk_idx in 0..inner.num_chunks() {
        for row_idx in 0..inner.num_rows_of_chunk(chk_idx) {
            ...
            let key = row_key_by(keys, |key| {
                let offset = if outer_is_left { key.right } else { key.left };
...
pub struct JoinExec<C: Columns> {
    ...
    /// Streaming state of the parallel exact-integer probe, created on the
    /// first dispatched chunk of a probe phase and dropped with the hash
    /// state.
    parallel_probe: Option<ParallelProbePipeline<C>>,
...
impl<C: Columns + Clone + Send + Sync + 'static> JoinExec<C> {
    ...
    pub(crate) fn set_default_values(&mut self, values: Vec<Datum>) -> Result<(), ExecError> {
        self.output
            .set_default(self.kind == JoinKind::Right, &values)
    }

    pub(crate) fn set_output_offsets(&mut self, offsets: Vec<usize>) -> Result<(), ExecError> {
        self.output = JoinOutput::resolved(
            &offsets,
            self.kind,
            self.left_types.len(),
    ...
    pub fn is_hash_join(&self) -> bool {
        self.native_hash || !self.keys.is_empty()
    }
    ...
    fn padded_row(&self, outer_row: &[Datum]) -> Vec<Datum> {
        self.output.padded(self.outer_is_left(), outer_row)
    }

    fn append(&self, req: &mut Chunk, joined: &[Datum]) {
        self.output.datums(req, joined);
    }
    ...
    /// Resume one outer row's probe, stopping at the caller's chunk boundary.
    /// The key, match flag and residual scratch survive subsequent Next calls.
    fn emit_outer_chunk_rows<'a, I>(
        &self,
        req: &mut Chunk,
        outer_row: Row<'_>,
        candidates: I,
        probe: &mut IndexRowProbe,
        mut scratch: Option<&mut Chunk>,
    ) -> Result<bool, ExecError>
    where
    ...
    pub(crate) fn set_index_hash_join(&mut self, keep_outer_order: bool) {
        self.index_hash = Some(keep_outer_order);
    }
    ...
    /// One call fills the requested output across task boundaries or returns
    /// the final partial chunk at EOF, like Go IndexLookUpJoin.Next.
    ...
    fn fill_index_batch(&mut self, required_rows: usize) -> Result<bool, ExecError> {
        if self.index_state.is_none() {
            ...
            self.index_state = Some(IndexLookupState {
                ...
                cursor: 0,
                probe: None,
                condition_scratch: (self.keys.is_empty() || !self.residual_conditions.is_empty())
                    .then(|| Chunk::new_with_capacity(&self.condition_types, 1)),
                inner: List::new(
                    &inner_types,
                    self.meta.init_cap(),
        ...
        loop {
            ...
            if state.hash_results.is_some()
                || state.hash_chunk.is_some()
    ...
    fn load_index_batch(&mut self) -> Result<(), ExecError> {
        ...
        if state
            .unordered
            .as_ref()
            .is_some_and(|tasks| tasks.active > 0)
        {
            let tasks = state.unordered.as_mut().expect("unordered hash variant");
            let outcome = tasks.recv()?;
            if let IndexTaskOutcome::HashChunk(chunk) = outcome {
                state.hash_chunk = Some(chunk);
                return Ok(());
            }
            return match outcome {
                IndexTaskOutcome::HashFinished {
                    ...
                    condition_evals.set(condition_evals.get() + evaluations);
                ...
                _ => Err(ExecError::internal(
                    "unexpected unordered index-hash result",
                )),
            };
        }
        ...
        // Ordered tasks retain N-1 later lookups while the current one drains.
        Self::fill_index_task_queue(
            ctx,
            outer_child,
            keys,
        ...
        match task.source {
            PendingIndexLookupSource::Draining { results } => {
                let outcome = results.recv().map_err(|_| {
                    // No outcome returned ownership of the outer rows. The
                    // pending task is already removed, so Close cannot release
                    // this charge through its pending/current task counters.
                    tracker.consume(-task.outer_bytes);
                    ExecError::internal("the index-join inner worker stopped without a result")
                })?;
                match outcome {
                    IndexTaskOutcome::HashChunk(chunk) => {
                        state.hash_task_bytes = task.outer_bytes;
                        state.hash_results = Some(results);
                        state.hash_chunk = Some(chunk);
                        Ok(())
                    }
                    IndexTaskOutcome::HashFinished {
                    ...
                    IndexTaskOutcome::Failed { outer, error } => {
                        state.outer = outer;
                        Err(*error)
                    }
                }
    ...
    fn fill_index_task_queue(
            ...
            < target
        {
            ...
            let source = if state.prefetch_disabled {
                Some(PendingIndexLookupSource::Synchronous(index_task_probes(
                    ctx,
                    keys,
                    &shared.probe_plan,
                    &outer,
                ...
                // Go's inner worker prepares the lookup and map. Output
                // remains on the consumer, so LIMIT never materializes a
                // task's complete join fanout merely to read its first row.
                let (tx, source) = if let Some(tasks) = state.unordered.as_mut() {
                    (tasks.admit(outer_bytes), None)
                } else {
                    let (tx, results) = std::sync::mpsc::sync_channel(1);
                    (
                        IndexResultSender::from(tx),
    ...
    fn drain_index_batch(&mut self, req: &mut Chunk) -> Result<(), ExecError> {
        // Keep mutable cursor ownership separate from the join's immutable
        // expression/output context, restoring it on both success and error.
        let mut state = self
            .index_state
            .take()
            .expect("a current index batch exists");
        let result = if state.hash_results.is_some() || state.hash_chunk.is_some() {
            self.drain_index_hash_results(req, &mut state)
        } else if let Some(output) = self.index_hash_output() {
            if state.hash.is_none() {
                match IndexHashState::new(
                    &output,
                    &self.keys,
                    &state.outer,
                    &state.inner,
                    ...
                    Ok(hash) => state.hash = Some(hash),
                    Err(error) => {
                        self.index_state = Some(state);
                        return Err(error);
                    }
                }
                state.matched.clear();
            ...
            self.drain_index_state(req, &mut state)
        };
        self.index_state = Some(state);
        result
    }

    fn drain_index_hash_results(
        &mut self,
        req: &mut Chunk,
        state: &mut IndexLookupState,
    ) -> Result<(), ExecError> {
        while !req.is_full() {
            if let Some(chunk) = state.hash_chunk.as_mut() {
                // Go handleResult swaps columns and returns the emptied
                // result buffer to its worker. Small requests retain a cursor.
                if req.num_rows() == 0
                    && chunk.row == 0
                    && chunk.chunk.num_rows() <= req.required_rows()
                {
                    req.swap_columns(&mut chunk.chunk);
                    state.hash_chunk = None;
                    return Ok(());
                }
                let count =
                    (req.required_rows() - req.num_rows()).min(chunk.chunk.num_rows() - chunk.row);
                req.append_range_from(&chunk.chunk, chunk.row, chunk.row + count);
                chunk.row += count;
                if chunk.row == chunk.chunk.num_rows() {
                    state.hash_chunk = None;
                }
                if req.is_full() {
                    return Ok(());
            ...
            // Unordered chunks arrive through the shared task queue in
            // load_index_batch; never wait on the previous chunk's worker.
            if state.hash_results.is_none() {
                return Ok(());
            }
            let outcome = state
                .hash_results
            ...
            match outcome {
                ...
                IndexTaskOutcome::HashFinished {
                    ...
                    self.tracker
                        .consume(-std::mem::take(&mut state.hash_task_bytes));
                    state.hash_results = None;
                    self.condition_evals
                        .set(self.condition_evals.get() + evaluations);
                    if let Some(IndexLookupPlan {
                        source: IndexLookupSource::Leaf(leaf),
                        ..
                    }) = &self.index_lookup
                    {
                        leaf.add_produced(inner_rows);
                ...
                IndexTaskOutcome::Failed { outer, error } => {
                    state.hash_task_bytes = 0;
                    state.outer = outer;
                    state.hash_results = None;
                    return Err(*error);
                }
                _ => return Err(ExecError::internal("unexpected index-hash worker result")),
            }
        }
        Ok(())
    }

    fn drain_index_state(
        &self,
        req: &mut Chunk,
        state: &mut IndexLookupState,
    ) -> Result<(), ExecError> {
        ...
        loop {
            ...
            if state.probe.is_none() {
                let key = row_key_by(&self.keys, |key| {
                    let offset = outer_offset(key);
                ...
                state.probe = Some(IndexRowProbe {
                    key,
                    next_inner: 0,
                    matched: false,
                });
            ...
            let positions = probe
                ...
                .map(Vec::as_slice)
                .unwrap_or_default();
            let candidates = positions[probe.next_inner..]
                .iter()
                .map(|ptr| state.inner.get_row(*ptr));
            let done = self.emit_outer_chunk_rows(
                req,
                outer_row,
                candidates,
                probe,
                state.condition_scratch.as_mut(),
            )?;
            if !done {
                return Ok(());
            }
    ...
    pub(crate) fn set_merge_plan(&mut self, plan: crate::merge_join_plan::MergeJoinPlan) {
        if matches!(
            ...
            // Native plans already carry precisely that residual list.
            if !self.conditions.is_empty() {
                ...
                self.residual_conditions =
                    self.conditions
                        ...
                        .flat_map(crate::hash_join::split_conjuncts)
                        .filter(|condition| {
                            !crate::hash_join::equi_key(condition, width).is_some_and(|key| {
                                !key.null_safe
                                    && plan.keys.iter().any(|merge| {
                                        merge.left == key.left && merge.right == key.right
                                    })
                            })
                        })
                        .cloned()
            ...
            MergePendingOutput::Matched {
                ...
                while *outer_index < outer.end && !req.is_full() {
                    let selected = {
                        ...
                        let side = if outer.side_left {
                            &state.left
                        } else {
                            &state.right
                        };
                        side.selected[*outer_index]
                    };
                    if !selected {
                        *inner_ptr = None;
                    }
    ...
    /// Go MergeJoinExec.Next: retain one spillable inner group, stream
    /// outer groups, and resume their output cursor at the requested bound.
    fn next_merged(&mut self, req: &mut Chunk) -> Result<(), ExecError> {
        ...
        loop {
            ...
            let inner = state.inner_group.as_mut().expect("inner group installed");
            let (
                inner_side,
                inner_child,
                inner_keys,
                ...
                (
                    ...
                    self.left
                        .as_deref_mut()
                        .ok_or_else(|| ExecError::internal("merge join has no left child"))?,
                    &left_keys,
                    &left_types,
                    &mut state.right,
                    self.right
                        .as_deref_mut()
                        .ok_or_else(|| ExecError::internal("merge join has no right child"))?,
                    &right_keys,
                    &right_types,
                )
            } else {
            ...
            // Go fetches the inner group first, then the outer group, and
            // stops as soon as the outer stream is exhausted.
            if inner_side.group_len == 0 {
    ...
    ///
    /// The current slice remains deliberately narrow: one ordinary integer
    /// equality key and at most one build match per key. Inner joins may carry
    /// any residual expression because Go evaluates `OtherConditions` on the
    /// probe workers through its ordinary expression evaluator. Preserved join
    /// families remain on the serial path when they have residuals until their
    /// worker match-state protocol is ported too.
    ...
    fn next_parallel_exact_int_hashed(&mut self, req: &mut Chunk) -> Result<(), ExecError> {
        loop {
            ...
            if in_flight == 0 && self.hash.as_ref().is_some_and(|hash| hash.probe_done) {
                if self.hash_builds_preserved_side() {
                    self.drain_preserved_build_rows(req)?;
                }
                return Ok(());
            }
            self.advance_parallel_exact_int_probe()?;
        }
    }
    ...
    /// One step of Go's probe pipeline on the session thread. In order: hand
    /// finished results to the parent (`Next` on `joinResultCh`), then fetch
    /// and dispatch one probe chunk while a probe chunk and a result chunk are
    /// free (`fetchProbeSideChunks` blocks on `probeChkResourceCh` otherwise),
    /// else wait for one worker to finish. Fetching overlaps probing exactly
    /// as Go's fetcher goroutine overlaps its join workers; the child is still
    /// driven by this thread only.
    fn advance_parallel_exact_int_probe(&mut self) -> Result<(), ExecError> {
        debug_assert!(self.can_parallelize_exact_int_probe());
        self.ensure_parallel_probe_pipeline();
        self.collect_parallel_probe_results(false)?;
        if self
            .hash
            .as_ref()
        ...
        let in_flight = self
            ...
            .map_or(0, |pipeline| pipeline.in_flight);
        if in_flight > 0 {
            self.collect_parallel_probe_results(true)?;
        }
        Ok(())
    }

    /// Takes the per-phase snapshots once, on the first dispatched chunk.
    fn ensure_parallel_probe_pipeline(&mut self) {
        if self.parallel_probe.is_some() {
            return;
        }
        ...
        self.parallel_probe = Some(ParallelProbePipeline {
    ...
    /// blocking when asked), then releases results in source-chunk order.
    /// Only this thread mutates the build side's matched bitmap.
    fn collect_parallel_probe_results(&mut self, block: bool) -> Result<(), ExecError> {
        let Some(pipeline) = self.parallel_probe.as_mut() else {
            return Ok(());
        };
        ...
        let condition_evals = released.iter().fold(0u64, |total, result| {
            ...
            if batch_ptrs.len() == input.num_rows() {
                ...
                if preserved_only {
                    if matches!(kind, JoinKind::Semi) && !builds_preserved {
                        ...
                        output_layout.side_range(&mut output, true, &probe_compact);
                        output_layout.finish_range(&mut output, probe_compact.num_rows());
                    }
                    if builds_preserved {
                ...
                let build_key_from_probe = build_types.len() == 1
                    ...
                    && !build_types[0].is_hybrid()
                    && !probe_types[key_offset].is_hybrid();
    ...
    fn drain_chunk_backed_probe(
        ...
        while !req.is_full() {
            ...
            let probe_row = hash.probe_chunk.get_row(hash.probe_row);
            if hash.probe_candidate_idx == 0 && hash.probe_candidates.is_empty() {
                let selected =
            ...
            if hash.probe_candidate_idx == hash.probe_candidates.len() {
                if !hash.probe_matched && !builds_preserved {
                    match kind {
                        JoinKind::Left | JoinKind::Right => self.output.unmatched(
                            req,
                            probe_is_left,
                            probe_row,
                            hash.build_types.len(),
                        ),
                        JoinKind::AntiSemi => self.output.preserved(req, probe_row),
                        JoinKind::LeftOuterSemi => {
                            self.output.preserved(req, probe_row);
                            req.append_datum(self.output.width(), &Datum::Int(0));
                        }
    ...
    fn drain_chunk_backed_residual_probe(
        ...
        loop {
            ...
            let (probe_index, ptr) = {
                ...
                if !hash.probe_selected.is_empty() && !hash.probe_selected[hash.probe_row] {
                    hash.probe_row += 1;
                    continue;
                }
                if hash.probe_candidate_idx >= hash.probe_candidates.len() {
...
/// Keep Go's Selection/inner-reader boundary chunk-backed. Compose with an
/// existing physical-row selection, preserving its order and duplicates.
/// Most batches pass unchanged and do not allocate a selection vector.
fn filter_index_inner_chunk(chunk: &mut Chunk, offsets: &[usize]) -> Result<(), ExecError> {
    if offsets.is_empty() {
        return Ok(());
    }
    let mut selected = None;
    for index in 0..chunk.num_rows() {
        let physical = |index| chunk.sel().map_or(index, |sel| sel[index]);
        if chunk_row_non_null_at(chunk.get_row(index), offsets)? {
            if let Some(selected) = selected.as_mut() {
                Vec::push(selected, physical(index));
            }
        } else if selected.is_none() {
            selected = Some((0..index).map(physical).collect::<Vec<_>>());
        }
    }
...
impl<C: Columns + Clone + Send + Sync + 'static> Executor for JoinExec<C> {
    ...
    fn close(&mut self) -> Result<(), ExecError> {
        ...
        if let Some(hash) = self.hash.as_mut() {
            if let Some(table) = Arc::get_mut(&mut hash.table) {
                table.close();
            }
        }
        ...
        if let Some(state) = self.index_state.take() {
            let current = state.outer.bytes + state.inner_bytes + state.hash_task_bytes;
            let pending = state
                .pending
                .iter()
