...
func TestRestoreBindingTextRunsEveryTime(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer db.Close()
	for range 2 {
		mock.ExpectExec(`ALTER TABLE mysql\.bind_info MODIFY COLUMN bind_sql TEXT NOT NULL`).
			WillReturnResult(sqlmock.NewResult(0, 0))
	}
	operation, found := findRollbackOperation(newRunner(db, nil, nil).rollbackOperations(), "restore mysql.bind_info.bind_sql to TEXT")
	require.True(t, found)
	require.NoError(t, operation.apply(context.Background()))
	require.NoError(t, operation.apply(context.Background()))
	require.NoError(t, mock.ExpectationsWereMet())
