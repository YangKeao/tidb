...
func (r *runner) apply(ctx context.Context, force bool) error {
	initial, err := r.preflight(ctx)
	if err != nil {
		return err
	}
	initial.print(r.out)
	if err := r.acceptPreflight(initial, force); err != nil {
		return err
	}
...
func (r *runner) acceptPreflight(report *report, force bool) error {
	preflightErr := report.error()
	if preflightErr == nil {
		return nil
	}
	if !force {
	...
	fmt.Fprintf(r.out, "WARNING: --force ignores %d preflight blocker(s); continuing with apply\n", len(report.blockers))
	return nil
}
